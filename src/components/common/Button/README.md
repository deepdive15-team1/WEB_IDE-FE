...
`startIcon` 또는 `endIcon` props에 아이콘 컴포넌트를 전달하면 자동으로 간격이 조정됩니다.
...
#### Icon Only (아이콘 전용)
텍스트 없이 아이콘만 있을 때 사용하며, **정사각형 비율**을 유지합니다.
...
| `startIcon` | `ReactNode` | - | 버튼 텍스트 **앞쪽**에 배치될 아이콘 요소입니다. |
| `endIcon` | `ReactNode` | - | 버튼 텍스트 **뒤쪽**에 배치될 아이콘 요소입니다. |
